...
exports.getBallot = async (req, res) => {
  try {
    ...
    // Get all positions - ALWAYS show positions and approved candidates
    // This allows voters to see what's available even before/after voting windows
    const now = new Date();
    ...
    // Get ALL positions (removed time filtering so voters can always see positions)
    ...
    console.log(`Backend getBallot - found ${positions.length} total positions (no time filtering)`);
