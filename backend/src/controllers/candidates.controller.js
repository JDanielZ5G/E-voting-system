...
// Update nomination (Candidate)
exports.updateNomination = async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;

    // Check if nomination exists and belongs to user
    ...
    if (candidate.userId !== userId) {
      return res.status(403).json({ error: 'Unauthorized to update this nomination' });
    }

    // Check if nomination window is open (optional: strictly enforce or allow updates for corrections?)
    // For now, we'll allow updates if status is SUBMITTED or REJECTED, regardless of time, 
    // to allow fixing mistakes as requested by user.
    if (candidate.status === 'APPROVED') {
      return res.status(400).json({ error: 'Cannot update an approved nomination' });
    }
    ...
    // Handle file updates
    if (req.files?.manifesto?.[0]) {
      // Delete old manifesto
      if (candidate.manifestoUrl) {
        try {
          const oldPath = path.join(__dirname, '../../', candidate.manifestoUrl);
          await fs.unlink(oldPath).catch(() => { });
        } catch (e) {
          console.warn('Failed to delete old manifesto');
        }
      }
      dataToUpdate.manifestoUrl = `/uploads/${req.files.manifesto[0].filename}`;
    }
    ...
    if (req.files?.photo?.[0]) {
      ...
      if (candidate.photoUrl) {
        try {
          const oldPath = path.join(__dirname, '../../', candidate.photoUrl);
          await fs.unlink(oldPath).catch(() => { });
        } catch (e) {
          console.warn('Failed to delete old photo');
        }
      }
      dataToUpdate.photoUrl = `/uploads/${req.files.photo[0].filename}`;
    }
